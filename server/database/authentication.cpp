...
int new_token(const std::string &name, const std::string &notes) {
  ...
  std::string sql = "INSERT INTO auth (name, token, notes) VALUES (?, base32_encode(sha256(? || ?)), ?) "
                    "ON CONFLICT (name) DO UPDATE "
                    "SET token = base32_encode(sha256(? || ?))";
  ...
  if (sqlite3_bind_text(stmt, 1, name.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK   ||
      ...
      sqlite3_bind_text(stmt, 5, token.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK  ||
      sqlite3_bind_text(stmt, 6, shared_resources::db_salt.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK  ||
      (!notes.empty() && sqlite3_bind_text(stmt, 7, notes.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK)
  ) {
