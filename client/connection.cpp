...
void service_thread_func(std::atomic<bool> &flag_kill, std::queue<std::string> &user_id) {
  ...
  while (!flag_kill) {
    ...
    if (host_status == -1) { std::cerr << "[Error] Unable to connect to host (connection)\n"; exit(1); }
...
void proxy_thread_func(std::atomic<bool> &flag_kill, int host_fd, sockaddr_in host_addr, int service_fd) {
  ...
  while (!flag_kill) {
    ...
    if (ready_for_call < 0) {
      ...
      if (send(host_fd, buffer, nbytes, 0) < 0) { std::cerr << "[Warning] Unable to send buffer to user (connection:proxy_thread)\n"; break; }
    }
